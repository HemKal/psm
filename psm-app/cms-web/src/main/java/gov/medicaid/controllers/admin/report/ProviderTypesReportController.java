...
public class ProviderTypesReportController extends gov.medicaid.controllers.BaseController {
    ...
    public ModelAndView getProviderTypes(
        ...
        List<ProviderType> providerTypes = getProviderTypesFromDB();
        ...
        try {
            ...
            for (Month month : months) {
                for (ProviderType providerType : providerTypes) {
                    if (month.containsEnrollments(providerType)) {
    ...
    private List<ProviderType> getProviderTypesFromDB() throws PortalServiceException {
        ...
        return providerTypeService.search(criteria).getItems();
    }
    ...
    public static class Month {
        Map<String, List<Enrollment>> typeEnrollments;
        LocalDate month;
        ...
        public Month(EnrollmentMonth em, ProviderEnrollmentService enrollmentService)
            ...
            for(Enrollment e : em.getEnrollments()) {
                ...
                if(providerType != null) {
                    if(!typeEnrollments.containsKey(providerType.getCode())) {
                        typeEnrollments.put(providerType.getCode(), new ArrayList<>());
                    }
                    typeEnrollments.get(providerType.getCode()).add(e);
                }
            }
        }

        public List<Enrollment> getEnrollments(ProviderType tp) {
            return typeEnrollments.get(tp.getCode());
        }

        public boolean containsEnrollments(ProviderType tp) {
            return typeEnrollments.containsKey(tp.getCode());
        }
